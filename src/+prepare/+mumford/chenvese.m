...
%   auto mask settings
  if ischar(mask)
      switch lower (mask)
          case 'small'
              mask = prepare.mumford.maskcircle2(I,'small');
          case 'medium'
              mask = prepare.mumford.maskcircle2(I,'medium');
          case 'large'
              mask = prepare.mumford.maskcircle2(I,'large');              
          case 'whole'
              mask = prepare.mumford.maskcircle2(I,'whole'); 
              %mask = init_mask(I,30);      
          case 'whole+small'
              m1 = prepare.mumford.maskcircle2(I,'whole');
              m2 = prepare.mumford.maskcircle2(I,'small');
...
switch lower(method)
        ...
        %-- Main loop
          for n=1:num_iter
              ...
              for i=1:layer
                  ...
                  c1 = sum(sum(L.*prepare.mumford.Heaviside(phi0)))/(length(inidx)+eps); % average inside of Phi0
                  c2 = sum(sum(L.*(1-prepare.mumford.Heaviside(phi0))))/(length(outidx)+eps); % verage outside of Phi0
              ...
              force = mu*prepare.mumford.kappa(phi0)./max(max(abs(prepare.mumford.kappa(phi0))))+1/layer.*force_image;
              ...
              phi0 = phi0+dt.*force;
              new = phi0;
              indicator = prepare.mumford.checkstop(old,new,dt);

              % intermediate output
              if(mod(n,20) == 0) 
                 prepare.mumford.showphi(I,phi0,n);  
              end;
    ...
    case 'multiphase'
        ...
        for n=1:num_iter
              ...
              f_image11=(L-c11).^2.*prepare.mumford.Heaviside(phi1).*prepare.mumford.Heaviside(phi2)+f_image11;
              f_image12=(L-c12).^2.*prepare.mumford.Heaviside(phi1).*(1-prepare.mumford.Heaviside(phi2))+f_image12;
              f_image21=(L-c21).^2.*(1-prepare.mumford.Heaviside(phi1)).*prepare.mumford.Heaviside(phi2)+f_image21;
              f_image22=(L-c22).^2.*(1-prepare.mumford.Heaviside(phi1)).*(1-prepare.mumford.Heaviside(phi2))+f_image22;
              end
              ...
              % curvature on phi1
              curvature1 = mu*prepare.mumford.kappa(phi1);
              ...
              % curvature on phi2
              curvature2 = mu*prepare.mumford.kappa(phi2);
              ...
              if indicator 
                 prepare.mumford.showphi(I, new, n);
                 %make mask from SDF
              ...
              % re-initializations
              phi1 = prepare.mumford.reinitialization(phi1, 0.6);%sussman(phi1, 0.6);%
              phi2 = prepare.mumford.reinitialization(phi2, 0.6);%sussman(phi2,0.6);

              %intermediate output
              if(mod(n,20) == 0) 
                 phi(:,:,1) = phi1;
                 phi(:,:,2) = phi2;
                 prepare.mumford.showphi(I, phi, n);
              end;
          end;
          phi(:,:,1) = phi1;
          phi(:,:,2) = phi2;
