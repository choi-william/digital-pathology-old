...
I = imread('+prepare/+mumford/brain.jpg');
% Customerlized Mask
m = zeros(size(I,1),size(I,2));
m(20:120,20:120) = 1;
seg = prepare.mumford.chenvese(I,m,500,0.1,'chan'); % ability on gray image
% Built-in Mask
seg = prepare.mumford.chenvese(I,'medium',400,0.02,'chan'); % ability on gray image
%-- End 
...
close all
clear all
P = imread('+prepare/+mumford/anti-mass.jpg');
...
seg = prepare.mumford.chenvese(I,'large',300,0.02,'chan'); 

% Chan & Vese for vector image works here
seg = prepare.mumford.chenvese(I,'large',300,0.02,'vector');
% Using built-in mask = 'whole' leads faster and better segmentation
seg = prepare.mumford.chenvese(I,'whole',800,0.02,'vector');
...
I = imread('+prepare/+mumford/4colors.jpg');
seg = prepare.mumford.chenvese(I,'whole',100,0.1,'multiphase'); 
%-------------------------------------------
% example on real image
close all
clear all

I = imread('+prepare/+mumford/flowers.jpg');
seg = prepare.mumford.chenvese(I,'whole',400,0.2,'multiphase'); 
%-- End
