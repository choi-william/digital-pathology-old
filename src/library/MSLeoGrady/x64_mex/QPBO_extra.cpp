...
inline int QPBO::DetermineSaturation(Node* i)
{
	Arc* a;
	int c1 = -i->tr_cap;
	int c2 = i->tr_cap;
...
bool QPBO::Probe(int* mapping)
{
	...
	while ( 1 )
	{
		...
		for (i_index=list.GetNext(-current_list) ; i_index>=0; i_index=i_index_next)
		{
			...
			int INFTY0 = DetermineSaturation(i);
			int INFTY1 = DetermineSaturation(_i[1]);
			int INFTY = ((INFTY0 > INFTY1) ? INFTY0 : INFTY1) + 1;
